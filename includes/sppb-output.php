...
function sppb_generate_links( $args ) {
        ...
        if ( isset($args['show']) ) {
        	$show  = strtolower($args['show']);
	        $show  = preg_replace('/[^a-z]+/', "", $show);
        }
        ...
        if ( isset($args['property']) ) {
	        ...
	        $property  = preg_replace('/[^a-z]+/', "", $property);
        }


		$options = array( 'show' => 'both', 'cat' => $cat, 'property' => $property);

		// Don't accept anything but 'both', 'previous' or 'next'
		$options['show'] = ( isset($show) && ('previous' == $show || 'next' == $show) ) ? $show : 'both';

		$result = sppb_generated_links( $options );

		return $result;

} // End sppb_generate_links
	...
	if ( ( ! in_array( $current_category, $sppb['exclude_category'] ) ) && ( ! in_array( $current_product_id, $sppb['exclude_product'] ) ) ) {
 		...
 		if ( shopp('collection.has-products') ){
			...
			switch( $show ) {
				...
				case 'next':
                    if ( 'url' == $args['property'] ) return $next_product_link;
					...
					break;
				case 'previous':
                    if ( 'url' == $args['property'] ) return $previous_product_link;
					...
					break;
				default:
                    if ( 'url' == $args['property'] ) return array($previous_product_link, $next_product_link);
					$result = "<div id='sppb-previous'><a href='" . $previous_product_link . "'>";
